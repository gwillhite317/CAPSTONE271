 ...
 * version 1
 *
 * This Class tests the usage for semester ui
 */
...
public static void main(String[] args) {
        ...
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("MM/dd/yyyy");  // use a formatter to help with dob requirement
        ...
        String name = scanner.nextLine().trim();  //trim is used to eliminate any whitespace before and after the string

        String fullName = title + " " + name;  //Combine title and full name to search for Professor

        ProfessorE professor = professors.stream()
        .filter(p -> (p.getTitle().trim() + " " + p.getName().trim()).equalsIgnoreCase(fullName))
        .findFirst()
        .orElse(null);   //Used to search for the professor entered by the user
        ...
        List<Course> courses = selectedSemester.findCoursesByProfessor(professor);  //getting courses for the selected semester that the professor teaches
